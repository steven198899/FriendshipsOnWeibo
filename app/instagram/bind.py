...
def bind_method(**config):
    ...
    class InstagramAPIMethod(object):
        ...
        def __init__(self, api, *args, **kwargs):
            ...
            self.max_pages = kwargs.pop("max_pages", 1)
            self.parameters = {}
        ...
        def _do_api_request(self, url, method="GET", body=None, headers=None):
            ...
            if status_code == 200:
                ...
                return api_response, content_obj.get('pagination', {}).get('next_url')
            else:
        ...
        def generate_with_next_url(self, url):
            response = urllib.urlopen(url)
            #print 'next_url: ', response.read()
            json_content = simplejson.loads(response.read())
            if 'next_url' in json_content['pagination']:
              return json_content['data'], json_content['pagination']['next_url']
            else:
              return json_content['data'], None
        ...
        def execute(self):
            ...
            if self.next_url is not None:
                return self.generate_with_next_url(self.next_url)
